...
@RestController
@RequestMapping("/quantitative-check")
@RequiredArgsConstructor
public class QuantitativeCheckController {

    private static final Logger log = LoggerFactory.getLogger(QuantitativeCheckController.class);

    @Autowired
    private QuantitativeCheckService service;

    @PostMapping
    public ResponseEntity<ResponceData> createCheck(
            @Valid @RequestBody QuantitativeCheckDTO dto,
        ...
        try {
            ...
            QuantitativeCheck saved = service.saveCheck(dto, token);
            return ResponseEntity.ok(new ResponceData("success", 200, "QuantitativeCheck created", saved, 1));
    ...
    public ResponseEntity<ResponceData> getAllChecks(
        ...
        try {
            ...
            Page<QuantitativeCheck> result = service.getAllChecks(search, pageable);

            return ResponseEntity.ok(
                    new ResponceData("success", 200, "Fetched QuantitativeChecks", result.getContent(), (int) result.getTotalElements()));
    ...
    public ResponseEntity<ResponceData> getById(@PathVariable Long id) {
        try {
            Optional<QuantitativeCheck> check = service.getCheckById(id);
            return check.map(value ->
                            ResponseEntity.ok(new ResponceData("success", 200, "Fetched QuantitativeCheck", List.of(value), 1)))
                    .orElseGet(() -> ResponseEntity.status(HttpStatus.NOT_FOUND)
                            .body(new ResponceData("fail", 404, "QuantitativeCheck not found", Collections.emptyList(), 0)));
        } catch (Exception e) {
            log.error("Error fetching QuantitativeCheck: {}", e.getMessage(), e);
            return ResponseEntity.internalServerError().body(new ResponceData("fail", 500, e.getMessage(), Collections.emptyList(), 0));
        }
    }
    ...
    public ResponseEntity<ResponceData> updateCheck(
        ...
        try {
            ...
            String token = authHeader.replace("Bearer ", "");
            QuantitativeCheck updated = service.updateCheck(id, dto, token);
            return ResponseEntity.ok(new ResponceData("success", 200, "QuantitativeCheck updated", List.of(updated), 1));
    ...
    public ResponseEntity<ResponceData> deleteCheck(
        ...
        try {
            String token = authHeader.replace("Bearer ", "");
            QuantitativeCheck deleted = service.deleteCheck(id, token);
            return ResponseEntity.ok(new ResponceData("success", 200, "QuantitativeCheck deleted", List.of(deleted), 1));
